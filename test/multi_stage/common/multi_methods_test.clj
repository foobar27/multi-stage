...
(deftest basic-multi-method-test
  (let [hierarchy (mm/make-multi-method 'too-simple
                                        identity
                                        :default)
        too-simple (-> hierarchy
    ...
    (testing "Check basic dispatch"
      (are [dispatch-val the-fn]
          (= the-fn (mm/get-method too-simple hierarchy dispatch-val))
        ::a ::a-fn
        ::b ::b-fn))
    (testing "Remove a method"
      (is (= ::default-fn (mm/get-method too-simple hierarchy ::unknown))))))
...
(deftest isa?-multi-method-test
  (let [hierarchy (-> (h/make-hierarchy)
        ...
        shapes (-> (mm/make-multi-method 'shapes
                   ...
                   (mm/add-method :default ::default-fn))]
    (are [dispatch-val the-fn]
        (= the-fn (mm/get-method shapes hierarchy dispatch-val))
  ...
  (let [hierarchy (-> (h/make-hierarchy)
    ...
    (testing "In case of a conflict, an exception should be thrown"
      (is (thrown? java.lang.IllegalArgumentException
                   (mm/get-method bar hierarchy [::rectangle ::rectangle]))))
    (testing "The prefers method returns empty table"
      (is (= {} (mm/prefers bar))))
    (let [bar (mm/prefer-method bar hierarchy [::rectangle ::shape] [::shape ::rectangle])]
      (testing "Adding a preference to resolve it dispatches correctly"
        (is (= ::rectangle-shape-fn (mm/get-method bar hierarchy [::rectangle ::rectangle])))))))
