(ns multi-stage.base.parser
  (:require [multi-stage.base.ast :refer [->literal ->lambda ->let ->variable ->cons ->plus ->minus ->times ->divide ->apply ->if
...
(def ^:private elementwise-ctors
  {`cons   ->cons
   ...
   `symbol? ->symbol?
   `>      ->gt
   `multi-stage.base.interpreter/lift   ->lift ;; TODO this should be in a different namespace (e.g. multi-stage.base.language)
   `true   ->TRUE
   `false  ->FALSE
   `nil    ->NIL})
